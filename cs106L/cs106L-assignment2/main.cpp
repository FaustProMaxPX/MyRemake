...
int numCommonLinks(const unordered_set<string>& curr_set, const unordered_set<string>& target_set) {
    ...
    int res = std::count_if(curr_set.begin(), curr_set.end(), [&target_set](string s) -> bool{return target_set.count(s) != 0;});
    return res; 
}
...
vector<string> findWikiLadder(const string& start_page, const string& end_page) {
    ...
    auto cmp_fn = [&w, &target_set](const vector<string>& left, const vector<string>& right) {
        ...
        int same1 = numCommonLinks(w.getLinkSet(page1), target_set);
        int same2 = numCommonLinks(w.getLinkSet(page2), target_set);
        return same1 < same2; // replace this line! make sure to use numCommonLinks.
    };
    // END STUDENT CODE HERE
    ///////////////////////////////////////////////////////////////////////////////////////////////////
